...
class MultiTaskHeads(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        Parameters
        ----------
        x : (B, in_features) for seq2point
            or (B, seg_size, in_features) for seq2seg (DU-NILM style)

        Returns
        -------
        powers : (B, N_app) or (B, seg_size, N_app)
        states : (B, N_app) or (B, seg_size, N_app)
        gated  : (B, N_app) or (B, seg_size, N_app)
        """
        # nn.Linear broadcasts over all leading dims — works for both 2D and 3D
        powers = torch.cat(
            [head(x) for head in self.power_heads], dim=-1
        )  # (B, N_app) or (B, seg_size, N_app)
        ...
        # Physical gating
        gated = powers * torch.sigmoid(states)
